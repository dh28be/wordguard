...
class userChatter(db.Model):
    ...
    profile_image = db.Column(db.String(100), nullable=False)  # 프로필 사진 경로 추가
...
def login_controller():
    ...
    selected_profile = request.form['selectedProfile']  # 선택된 프로필 정보를 가져옴
    if user_username.strip():
        existing_user = userChatter.query.filter_by(username=user_username).first()
        if existing_user:
            return redirect(url_for("profile", username=user_username))
        else:
            try:
                new_user = userChatter(username=user_username, profile_image=selected_profile)
...
def handle_message(data):
    ...
    socketio.emit("response", {
        ...
        "profile_image": data["profile_image"]  # 프로필 이미지를 포함하여 전송
    })
